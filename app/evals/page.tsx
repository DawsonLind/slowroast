...
export default async function EvalsPage() {
  ...
  return (
    <div className="flex min-h-full flex-col bg-background text-foreground">
      <SiteHeader subtitle="eval dashboard" />
...
function EvalsFooter() {
  return (
    <footer className="border-t border-border">
      <div className="mx-auto flex w-full max-w-5xl items-center justify-center px-6 py-4 text-xs text-muted-foreground">
        <span>
  ...
  return (
    <div className="flex min-w-0 flex-col gap-2">
      ...
      <ul className="flex flex-col divide-y divide-border text-xs">
        {ids.map((id, i) => (
          <li key={i} className="flex flex-col gap-0.5 py-1.5 sm:flex-row sm:items-baseline sm:gap-3">
            <span className="shrink-0 tabular-nums text-muted-foreground sm:w-10">
              Run {i + 1}
            </span>
            <span className="min-w-0 break-all font-mono">
              ...
              {featureIds[i] ? (
                <>
                  ...
                  <span className="text-muted-foreground">{featureIds[i]}</span>
                </>
              ) : null}
            </span>
          </li>
    ...
    <div className="flex min-w-0 flex-col gap-2">
      ...
      <div className="flex flex-col gap-2">
        {runs.map((r) => (
          <div
            key={r.index}
            className="flex flex-col gap-1.5 rounded-lg border border-border px-3 py-2 text-xs"
          >
            ...
            {r.status === "success" ? (
              r.topPriority ? (
                <div className="break-all font-mono text-muted-foreground">
                  ...
                  {r.topPriority.vercelFeatureId}
                </div>
              ) : (
                <div className="text-muted-foreground">no top priority</div>
              )
            ) : (
              <div className="break-all font-mono text-destructive">
        ...
        {features.map((id) => {
          ...
          if (!feature) {
            return (
              <span
                key={id}
                className="break-all rounded-full bg-destructive/10 px-2 py-0.5 font-mono text-destructive"
              >
                {id} (unknown)
              </span>
            );
          ...
          return (
            <a
              ...
              target="_blank"
              rel="noreferrer"
              className="break-all rounded-full bg-muted px-2 py-0.5 font-mono hover:bg-muted/60"
              title={feature.title}
            >
              {id}
            </a>
